...
#Region " DEVICE TESTS: OPEN, CLOSE, CHECK SUSBSYSTEMS "
    ...
    Public Sub OpenCloseSessionTest()
        Using device As Device = Device.Create
            DeviceTests.OpenSession(device)
            DeviceTests.CloseSession(device)
        End Using
    End Sub
    ...
    Public Sub OpenSessionTest()
        Using device As Device = Device.Create
            DeviceTests.OpenSession(device)
            DeviceTests.CheckSessionInfo(device)
            DeviceTests.CheckStatusSubsystemInfo(device)
            DeviceTests.CheckLocalNodeSubsystemInfo(device)
            DeviceTests.CheckSourceMeasureUnitInfo(device)
            DeviceTests.CloseSession(device)
        End Using
    End Sub
