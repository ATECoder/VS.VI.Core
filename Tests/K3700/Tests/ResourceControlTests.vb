...
#Region " DEVICE OPEN TEST "
    ...
    Private Shared Sub ConnectDevice(ByVal trialNumber As Integer, ByVal control As isr.VI.Instrument.ResourceControlBase)
        ...
        Using device As Device = New Device()
            ...
            control.ResourceName = TestInfo.ResourceName
            control.SelectedResourceName = TestInfo.ResourceName
    ...
    Public Sub OpenSessionTest()
        Using control As isr.VI.Instrument.ResourceControlBase = New isr.VI.Instrument.ResourceControlBase
            ResourceControlTests.ConnectDevice(1, control)
        End Using
    End Sub
    ...
    Public Sub OpenSessionTwiceTest()
        ...
        End Using
        Using control As isr.VI.Instrument.ResourceControlBase = New isr.VI.Instrument.ResourceControlBase
            ResourceControlTests.ConnectDevice(2, control)
        End Using
        Using control As isr.VI.Instrument.ResourceControlBase = New isr.VI.Instrument.ResourceControlBase
            ResourceControlTests.ConnectDevice(3, control)
            sw.Wait(TimeSpan.FromMilliseconds(100))
            ResourceControlTests.ConnectDevice(4, control)
        End Using
    End Sub
...
#Region " ASSIGNED DEVICE TESTS "
    ...
    Private Shared Sub AssignDevice(ByVal trialNumber As Integer, ByVal openFirst As Boolean, ByVal control As K3700Control, ByVal device As Device)
        ...
        Else
            ...
            actualBoolean = e.Cancel
            expectedBoolean = False
            Assert.AreEqual(expectedBoolean, actualBoolean, $"Open session {trialNumber} cancel; {e.Details}")
        End If

        actualBoolean = control.IsConnected
        expectedBoolean = True
        Assert.AreEqual(expectedBoolean, actualBoolean, $"Connector {trialNumber} is connected")

        actualBoolean = control.IsDeviceOwner
        expectedBoolean = False
        Assert.AreEqual(expectedBoolean, actualBoolean, $"Control {trialNumber} is not device owner")

        actualBoolean = control.SelectedResourceExists
        expectedBoolean = True
        Assert.AreEqual(expectedBoolean, actualBoolean, $"Connector {trialNumber} selected device exists")
    ...
    Public Sub AssignDeviceTest()
        Using control As isr.VI.Tsp.K3700.K3700Control = New isr.VI.Tsp.K3700.K3700Control
            Using Device As Device = Device.Create
                ResourceControlTests.AssignDevice(1, False, control, Device)
            End Using
        End Using
    End Sub
    ...
    Public Sub AssignOpenDeviceTest()
        Using control As isr.VI.Tsp.K3700.K3700Control = New isr.VI.Tsp.K3700.K3700Control
            Using Device As Device = Device.Create
                ResourceControlTests.AssignDevice(1, True, control, Device)
            End Using
        End Using
    End Sub
